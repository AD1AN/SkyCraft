...
bool ATestIsland::IsEdgeVertex(const FVector& Vertex, const TMap<int32, int32>& AxisVertexMap, int32 EdgeThickness)
{
...
bool ATestIsland::IsInsideShape(const FVector2D& Point, const TArray<FVector2D>& GeneratedShapePoints)
{
	int32 Crossings = 0;
...
void ATestIsland::GenerateIsland()
{
...
void ATestIsland::SetIslandSize(int32 NewSize)
{
	IslandSize = NewSize;
	OnRep_IslandSize();
	MARK_PROPERTY_DIRTY_FROM_NAME(ATestIsland, IslandSize, this);
}

void ATestIsland::OnRep_IslandSize()
{
	OnIslandSize.Broadcast();
}

TArray<FSS_DroppedItem> ATestIsland::SaveDroppedItems()
{
...
void ATestIsland::LoadDroppedItems(TArray<FSS_DroppedItem> SS_DroppedItems)
{
...
void ATestIsland::AddDroppedItem(ADroppedItem* DroppedItem)
{
	DroppedItems.Add(DroppedItem);
}

void ATestIsland::RemoveDroppedItem(ADroppedItem* DroppedItem)
{
	DroppedItems.Remove(DroppedItem);
}

void ATestIsland::AddConstellation(FSS_Astralon NewConstellation)
{
	SS_Astralons.Add(NewConstellation);
	MARK_PROPERTY_DIRTY_FROM_NAME(ATestIsland, SS_Astralons, this);
}

void ATestIsland::RemoveConstellation(FSS_Astralon RemoveConstellation)
{
...
void ATestIsland::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(ATestIsland, SS_Astralons, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ATestIsland, IslandSize, Params);
}