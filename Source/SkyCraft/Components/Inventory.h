...
public:
	...
	void Multicast_ChangeSlot(int32 SlotIndex, FSlot NewSlot);

	UFUNCTION(NetMulticast, Reliable, BlueprintCallable)
	void Multicast_EmptySlot(int32 SlotIndex);
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly)
	void DropIn(int32 SlotIndex, UInventory* DragInventory, int32 DragSlotIndex, uint8 DragQuantity);
	UFUNCTION(BlueprintCallable)
	bool CanDropIn(const FSlot& Slot);
	
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly)
	void Swap(int32 SlotIndex, UInventory* OtherInventory, int32 OtherSlotIndex);

	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly)
	void Spend(FSlot SpendSlot);
	
	UFUNCTION(BlueprintCallable, BlueprintPure)
	int32 CountItems(UDA_Item* DA_Item);

	UFUNCTION(BlueprintCallable, BlueprintPure)
	bool FindEmptySlots(int32 NumEmptySlots = 1);
	// GetEmptySlots() - maybe in the future, returns empty slots indexes.

	UFUNCTION(NetMulticast, Reliable, BlueprintCallable)
	void Multicast_AddProperty(int32 SlotIndex, FItemProperty NewProperty);

	UFUNCTION(NetMulticast, Reliable, BlueprintCallable)
	void Multicast_ChangeProperty(int32 SlotIndex, int32 PropertyIndex, FItemProperty NewProperty);
